...
test_that("can extract the mday from a Date", {
  x <- new_date(0)
  expect_identical(time_mday(x), 1L)
})

test_that("can extract the mday from an integer Date", {
  x <- structure(0L, class = "Date")
  expect_identical(time_mday(x), 1L)
})

test_that("can extract the mday from a Date subclass", {
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_mday(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(time_mday(x), NA_integer_)
})
...
test_that("can extract the mday from a POSIXlt", {
...
test_that("can extract the mday from a POSIXct in UTC", {
  expect_identical(time_mday(new_datetime_utc(0)), 1L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(time_mday(x), 31L)
  expect_identical(time_mday(y), 1L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), time_mday(x)),
    1L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), time_mday(x)),
    31L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_mday(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(time_mday(1), regexp = "Unknown object")
  expect_error(time_mday("x"), regexp = "Unknown object")
})
