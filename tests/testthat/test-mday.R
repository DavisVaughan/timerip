...
test_that("can extract the mday from a Date", {
  x <- new_date(0)
  expect_identical(rip_mday(x), 1L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_mday(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(rip_mday(x), NA_integer_)
})
...
test_that("can extract the mday from a POSIXct in UTC", {
  expect_identical(rip_mday(new_datetime_utc(0)), 1L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(rip_mday(x), 31L)
  expect_identical(rip_mday(y), 1L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), rip_mday(x)),
    1L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), rip_mday(x)),
    31L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_mday(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(rip_mday(1), regexp = "Unknown object")
  expect_error(rip_mday("x"), regexp = "Unknown object")
})
