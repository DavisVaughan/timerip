...
test_that("can extract the month from a Date", {
  x <- new_date(0)
  expect_identical(time_month(x), 1L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_month(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(time_month(x), NA_integer_)
})
...
test_that("can extract the month from a POSIXct in UTC", {
  expect_identical(time_month(new_datetime_utc(0)), 1L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(time_month(x), 12L)
  expect_identical(time_month(y), 1L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), time_month(x)),
    1L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), time_month(x)),
    12L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_month(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(time_month(1), regexp = "Unknown object")
  expect_error(time_month("x"), regexp = "Unknown object")
})
