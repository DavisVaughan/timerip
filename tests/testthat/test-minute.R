...
test_that("can extract the minute from a Date", {
  x <- new_date(0)
  expect_identical(time_minute(x), 0L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_minute(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(time_minute(x), NA_integer_)
})
...
test_that("can extract the minute from a POSIXlt", {
  ...
  expect_identical(time_minute(x), 23L)
})
...
test_that("can extract the minute from a POSIXct in UTC", {
  expect_identical(time_minute(new_datetime_utc(5000)), 23L)
})
...
test_that("20 minute DST gaps are handled correctly", {
  ...
  expect_identical(time_minute(x), 59L)
  expect_identical(time_minute(y), 20L)
})

test_that("bad date types are handled correctly", {
  x <- structure("x", class = c("POSIXct", "POSIXt"))
  expect_error(time_minute(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(time_minute(1), regexp = "Unknown object")
  expect_error(time_minute("x"), regexp = "Unknown object")
})
