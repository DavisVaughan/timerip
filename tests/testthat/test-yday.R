...
test_that("can extract the yday from a Date", {
  x <- new_date(0)
  expect_identical(time_yday(x), 1L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_yday(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(time_yday(x), NA_integer_)
})
...
test_that("can extract the yday from a POSIXct in UTC", {
  expect_identical(time_yday(new_datetime_utc(0)), 1L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(time_yday(x), 365L)
  expect_identical(time_yday(y), 1L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), time_yday(x)),
    1L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), time_yday(x)),
    365L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_yday(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(time_yday(1), regexp = "Unknown object")
  expect_error(time_yday("x"), regexp = "Unknown object")
})
