...
test_that("can extract the hour from a Date", {
  x <- new_date(0)
  expect_identical(rip_hour(x), 0L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_hour(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(rip_hour(x), NA_integer_)
})
...
test_that("can extract the hour from a POSIXlt", {
  ...
  expect_identical(rip_hour(x), 1L)
})
...
test_that("can extract the hour from a POSIXct in UTC", {
  expect_identical(rip_hour(new_datetime_utc(5000)), 1L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(rip_hour(x), 19L)
  expect_identical(rip_hour(y), 0L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), rip_hour(x)),
    0L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), rip_hour(x)),
    19L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_hour(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(rip_hour(1), regexp = "Unknown object")
  expect_error(rip_hour("x"), regexp = "Unknown object")
})
