...
test_that("can extract the wday from a Date", {
  x <- new_date(0)
  expect_identical(time_wday(x), 4L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_wday(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(time_wday(x), NA_integer_)
})
...
test_that("can extract the wday from a POSIXct in UTC", {
  expect_identical(time_wday(new_datetime_utc(0)), 4L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(time_wday(x), 3L)
  expect_identical(time_wday(y), 4L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), time_wday(x)),
    4L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), time_wday(x)),
    3L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(time_wday(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(time_wday(1), regexp = "Unknown object")
  expect_error(time_wday("x"), regexp = "Unknown object")
})
