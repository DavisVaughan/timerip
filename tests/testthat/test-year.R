...
test_that("can extract the year from a Date", {
  x <- new_date(0)
  expect_identical(rip_year(x), 1970L)
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_year(x), "`Date` type character")
})

test_that("Inf dates are handled correctly", {
  x <- structure(Inf, class = "Date")
  expect_identical(rip_year(x), NA_integer_)
})
...
test_that("can extract the year from a POSIXct in UTC", {
  expect_identical(rip_year(new_datetime_utc(0)), 1970L)
})
...
test_that("border times are handled correctly using their time zone", {
  ...
  expect_identical(rip_year(x), 1969L)
  expect_identical(rip_year(y), 1970L)
})
...
test_that("border times are handled correctly using system TZ", {
  ...
  expect_identical(
    with_envvar(list(TZ = "UTC"), rip_year(x)),
    1970L
  )

  expect_identical(
    with_envvar(list(TZ = "America/New_York"), rip_year(x)),
    1969L
  )
})
...
test_that("bad date types are handled correctly", {
  ...
  expect_error(rip_year(x), "`POSIXct` type character")
})
...
test_that("error if not a recognized date type", {
  expect_error(rip_year(1), regexp = "Unknown object")
  expect_error(rip_year("x"), regexp = "Unknown object")
})
