...
static SEXP date_rip_second(SEXP x);
static SEXP posixct_rip_second(SEXP x);
static SEXP posixlt_rip_second(SEXP x);

// [[ register() ]]
SEXP rip_second(SEXP x) {
  switch (time_class_type(x)) {
  case timerip_class_date: return date_rip_second(x);
  case timerip_class_posixct: return posixct_rip_second(x);
  case timerip_class_posixlt: return posixlt_rip_second(x);
...
#define DATE_RIP_SECOND(CTYPE, CONST_DEREF) {                    \
...
static SEXP date_rip_second(SEXP x) {
  ...
  switch (TYPEOF(x)) {
  case INTSXP: DATE_RIP_SECOND(int, INTEGER_RO); break;
  case REALSXP: DATE_RIP_SECOND(double, REAL_RO); break;
...
#undef DATE_RIP_SECOND

// -----------------------------------------------------------------------------

#define POSIXCT_RIP_SECOND(CTYPE, CONST_DEREF) {     \
  const CTYPE* p_x = CONST_DEREF(x);                 \
                                                     \
  for(R_xlen_t i = 0; i < size; i++) {               \
    stm tm;                                          \
...
static SEXP posixct_rip_second(SEXP x) {
  ...
  switch (TYPEOF(x)) {
  case INTSXP: POSIXCT_RIP_SECOND(int, INTEGER_RO); break;
  case REALSXP: POSIXCT_RIP_SECOND(double, REAL_RO); break;
